...
class Helper {
	 ...
	 * @return void
	 */
	public static function sptp_get_template( $template_name, $template_path = '', $default_path = '' ) {
		$cache_key = sanitize_key( implode( '-', array( 'template', $template_name, $template_path, $default_path, SPT_PLUGIN_VERSION ) ) );
		$template  = (string) wp_cache_get( $cache_key, 'wp-team-pro' );
		if ( ! $template ) {
			$template = self::sptp_locate_template( $template_name, $template_path, $default_path );
			wp_cache_set( $cache_key, $template, 'wp-team-pro' );
		}
